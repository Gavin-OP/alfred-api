...
> 以下为 AI 以「求职者 / HR / 资深求职中介」视角补充的候选工作流，待用户确认后转正。

- **多 Offer 对比与决策辅助**：同时手握多个 Offer 时，按薪资 / 城市 / 成长 / 风险等维度生成对比矩阵，辅助决策（不替用户做决定）。
- **入职前事务追踪**：Offer 后的体检、签证、毕业证明、租房等办理进度 tracking，临近节点提醒。
- **内推人 / 人脉关系维护**：对 referral / coffee chat 对象定期 follow-up、节日问候、进展同步，沉淀关系网。
- **技能缺口补强计划**：依据拒信与面试 gap 自动生成学习清单（关联 `skill` 与 `question_bank`）。
- **投递渠道效果分析**：统计各渠道（官网 / 内推 / 猎头 / 社媒）的投递→面试→Offer 转化率，指导精力分配。
