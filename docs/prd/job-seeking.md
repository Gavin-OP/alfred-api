...
1. **信息来源**（用户在哪里看到职位）：
    a. 邮件、公司官网、领英
    b. 别人转发的消息、小红书、微信公众号
    c. 朋友口头告知，或在其它地方看到并拍下的照片
2. **输入职位信息**（用户如何把信息交给 AI）：
    a. 公司官网链接、图片截图
    b. 语音输入、复制的文字消息等
3. **解析职位信息（Processing）**：AI 收到后解析，至少提取——
    a. 基本信息：公司、岗位类型、所在城市、截止日期
    b. 岗位要求：Requirements、所需技能、是否需要特定证书
    c. 投递要求：如何投递、是否需录制视频、是否需 CV / Cover Letter / Personal Statement 等（均为「按需可选」项，不一定强制；能解析出来就记录，解析不出不强求，绝不臆造——见 NFR-4）
4. **缺失補全与确认**：AI 解析后先做关键信息校验——若公司 / 岗位 / 截止日期等必填项缺失，主动追问用户补齐，直到关键信息齐全；随后将解析结果呈现给用户最终确认。
5. **入库并开始 tracking**：用户确认无误，职位正式入库并开始 tracking。
6. **记录用户想法与后续计划**：职位开始 tracking 后，用户可能围绕该职位产生想法，并通过语音 / 文字发给 AI。AI 解析并关联——
    a. **简历与经历**：若提到想用什么简历或经历，AI 解析这些经历是否已存在；若是新经历，解析后跟用户确认是否入库。
    b. **投递计划**：若提到想什么时候投递，AI 解析并设置一个日程提醒。
    c. **感想与面经**：若提到对公司的理解、想法或别人的面经，入库时需与**该公司**挂钩；若是对岗位的想法，则与**该岗位**挂钩——以便未来用户需要时，能随时调出与该公司或岗位相关的所有信息。
...
```mermaid
flowchart TD
  A[触发：工作流一结束后同一会话中用户继续投递，或用户另行指定岗位] --> B[检索并确认 JD 与历史想法]
  B --> C[信息汇总与提醒]
...
1. **上下文继承与确认（触发条件：在工作流一结束后的同一会话中、用户紧接着继续投递该岗位）**：系统自动继承**该岗位**上下文继续投递；否则（用户另行指定岗位或全新发起）需说明公司、岗位、城市等关键信息。系统检索并展示对应 JD 与用户此前围绕该岗位 / 公司的想法，与用户确认无误后再继续。
2. **信息汇总与提醒**：综合用户对该岗位的理解、对公司的评价、对城市的考量等，整理成一份总结，提醒用户："你要投递的岗位是这样的，你之前有这些想法。"
3. **简历与 Cover Letter 推荐（可迭代）**：
    a. 依据 JD 要求的技能点（Skill Set），从用户经历库中筛选并推荐最匹配的条目，用于优化简历。
    b. 若岗位需要 Cover Letter，同样推荐最匹配的经历素材。
    c. **迭代确认闭环**：用户对推荐结果给出反馈（采纳 / 替换 / 补充 / 排序调整），AI 据反馈重新推荐，如此往复直至用户给出「定稿」信号；循环次数由用户决定，无强制上限（承接 step 5 的版本生成）。
...
5. **动态编辑与生成**：
   ...
   (d) 若推荐或编辑时引用的关联条目（经历 / 技能 / 字段）缺失，将该字段标记「待补 / pending」并主动询问用户补充；待人工补齐后再继续生成，**绝不臆造替代内容**（见 NFR-4）。
6. **文件交付（CV / CL 命名规范）**：提供**可直接下载的文件**（本质上是一个指向生成文件的下载链接）。文件命名须专业规范——**全英文 + 下划线连接**，并明确区分 `CV` 与 `CL`，例如 `姓名_CV.pdf`、`姓名_CL.pdf`；严禁使用"第一版""第二版"等字眼。
...
```mermaid
flowchart TD
  A[用户告知投递的岗位] --> B[确认所用 CV / CL（官网投递则无邮件）]
...
1. **告知投递岗位**：用户告知已投递的岗位；投递渠道可能是官网投递、发消息或写邮件。
2. **确认所用材料**：用户确认本次投递所用的简历（CV）、Cover Letter 及邮件内容。注意：若为**官网投递**，往往没有邮件、而是在网站表单直接上传 CV/CL，此时「邮件内容」项不适用，所用材料以站点上传的版本为准。
3. **凭证与识别**：用户可能通过图片、PDF、语音或文字提供投递凭证；即便不提供原文、只说"刚才生成的那一份"，AI 也需识别并定位到对应版本并记录。

据此为该岗位记录**投递台账**（每项一行）：
   a. **投递状态**：是否已投递。
   b. **投递时间**。
   c. **所用材料**：本次投递的 CV 与 CL 版本；官网投递时即站点上传的版本。
   d. **投递方式及内容**：官网表单 / 邮件 / 聊天，以及具体内容。

**额外处理**（每项一行）：
   a. 若用户提到面经，解析后存入数据库，并与对应岗位 / 公司挂钩。
   b. 对特定岗位，可主动搜索相关信息（如预计收到第一轮面试的时间、准备建议等）告知用户；**搜不到时如实说明，绝不编造数据**（见 NFR-4）。
...
### 工作流四：面试信息追踪（Interview Tracking）
...
> 多轮面试的串联与轮次标记沿用工作流四的安排；本节仅补充面试链路中出现的**笔试**场景。
...
- **NFR-4 缺失信息·人工补齐优先**：编辑或生成过程中若发现关联信息缺失（经历库无匹配条目 / 某字段为空 / 引用了尚不存在的节点），AI 不得臆造或静默跳过；将该字段标记「待补 / pending」并主动向用户询问，待人工补充后再继续下游处理。本条是工作流九/十五「缺失即询问」与工作流三/四/八「不编造」的统一 generalization，专门堵住编辑 / 生成路径的幻觉口子。
